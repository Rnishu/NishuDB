...
type BNode []byte // no serialization required, can be dumped to disk
...
func (node BNode) getKey(idx uint16) []byte {
	...
	return node[pos+4:][:klen] //why not slice it at the same time?
}
...
func init() {
	node1max := HEADER + 8 + 2 + 4 + BTREE_MAX_KEY_SIZE + BTREE_MAX_VAL_SIZE // 8 is subtree pointer, 2 is offset, 4 is keylen and vallen(2 each)
